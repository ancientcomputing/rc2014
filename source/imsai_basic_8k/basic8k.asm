; May 2017
; Adapted to work with RC2014 BIOS
; Changes are Copyright Ben Chong and freely licensed back to the community
;---------------------------------------------------------
;Edits:
;    30-Jun-08 KJL
...
TBASE   EQU     9000h   ; Program load in RAM
...
; SETUP POINTERS FOR STACK, DATA, AND PROG
; INIT SIO BOARD
;
INIT1:
#if 1  
...
RUN4:   SHLD    ADDR1   ;SAVE ADDR
        ...
        RET             ;JUMP TO ROUTINE
RUN7:
        LHLD    ADDR1   ;RESTORE H,L POINTER
...
;        call RST2       ;SEE IF IT IS A NEXT STMT
        call    RST2
...
NEXT3:  LDA     FORNE   ;GET ORIG COUNT
        ...
        XCHG            ;PUT TO H,L
        SHLD    ADDR1   ;SAVR IT
;        call RST5       ;LOAD INDEX
        call    RST5
        POP     H       ;GET H,L (TBL)
        PUSH    H       ;RE-SAVE
        CALL    FADD    ;ADD STEP VALUE
        LXI     H,TVAR1 ;POINT TEMP AREA
;        call RST3       ;SAVE NEW INDEX
        call    RST3
        POP     H       ;GET H,L (TBL)
        PUSH    H       ;RE-SAVE
;        call RST4       ;GET LEN TO NEXT
        call    RST4
        DB      4
...
;        call RST5       ;GO LOAD IT
        call    RST5
        LHLD    ADDR1   ;GET ADDR OF INDEX
;        call RST3       ;GO STORE IT
        call    RST3
...
;        call RST4       ;POINT START OF ELEMENT 0,0
        call    RST4
        DB      -11 AND 0FFH
...
;        call RST4       ;MOVE TO NEXT COEFFICIENT
        call    RST4
        DB      -4 AND 0FFH
...
EXP1:   LXI     H,LN2E  ;POINT LN BASE 2 OF E
        ...
        call RST3       ;TEMP3=X*LN2E
        CALL    INT     ;FACC=INT(X*LN2E)
        LXI     H,TEMP4 ;POINT SAVE AREA
        call RST3       ;TEMP4=INT(X*LN2E)
        call RST3       ;DITTO FOR TEMP5
...
EV3:    ;     EQU     $       ;FUNCTIONS RETURN HERE
        ...
        PUSH    B       ;SAVE CTRS
        PUSH    H       ;SAVE ADDR
        call RST3       ;GO STORE THE VARIABLE
        MVI     A,0E3H  ;TYPE=NUM
...
GETS4:  INX     H       ;SKIP END QUOTE
        call RST1       ;SKIP TRAILING SPACES
...
; Call routine to write character to console
; Value in A
TODO1:
TESTO:  ;     EQU     $
        RST     1
...
;        call RST1       ;SKIP TO NON-BLANK
        ...
        MVI     E,' '   ;DEFAULT
        INX     H       ;POINT NEXT
;        call RST1       ;GET 2ND CHAR
        call    RST1
        CALL    NUMER   ;TEST IF NUMERIC
        JNZ     VAR2    ;BRIF NOT NUMERIC
        MOV     E,A     ;SAVE 2ND CHAR
        INX     H       ;POINT NEXT
;        call RST1       ;GET NON-BLANK FOLLOWING
        call    RST1
...
ULERR:
        ...
        DB      'UL',EM,FATAL   ;NOTE FATAL = CODE FOR RST 6
ZMERR:
        call    RST6;      EQU     $-1             ;LOG(X<=0),SQR(-X),0 DIVIDE
        DB      'OF',EM,FATAL
STERR:  ;   EQU     $-1             ;ERROR IN EXPRESSION STACK
        call    RST6
        DB      'ST',EM,FATAL
